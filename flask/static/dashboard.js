...
function updateMainTextPlaceholder(){
    $('#mainText').attr('placeholder', $('#conected').html() == "Crie uma senha" ? "Todos podem editar os arquivos desta coleção." : ( !permEdit ? "" : (!visitant_view_perm ? "Só você pode visualizar os arquivos desta coleção." : (!visitant_edit_perm ? "Todos podem visualizar esta coleção, mas só você pode editar seus arquivos." : "Todos podem editar os arquivos desta coleção.")) + ' Insira aqui o conteúdo' + (isMobile ? "" : " ou solte arquivos e imagens") + "."))
}
