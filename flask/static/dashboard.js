...
function triggerResize(first=false){
    ...
    if ($('#sidebar:hidden').length || $(window).width() < 600) {
        ...
        isMobile = true
        filename = $('#filename').attr('file')
        $('#troncoHomeLabel').html("<span class='mt-3 mb-2 ml-3' style='overflow-x: scroll; max-width:90%; width:100%; display:inline-block; white-space: nowrap; font-weight:bold;'>" + name + " > <span id='filenameMobile'>" + (special_files.indexOf(filename) == -1 ? filename : "") + "</span>" + "</span>")
        $('#troncoLogo').hide()
        $('.navbar-brand').hide()
...
function checkTheme(){
    ...
    elements = "#main, .prepend, .dropdown-toggle, .page-link, .page-item.active, .advancedSearchMetadataItem select, .advancedSearchMetadataItem input, #advancedSearchInput, .metadataItem, .metadataKey, .row, #recentFiles, #mainText, #historyMainText, #sidebar, html"
