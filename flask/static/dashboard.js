...
function updateToolbar(){
    ...
    if (checklist.length) {
        ...
        for (check in checklist) {
            $('[toolbar=checklist] .checklist-items').append('<div class="form-row checkbox-item-div align-items-left">' + (isMobile ? '<div class="mt-1 pl-2 mr-2 checklist-draggable" style="width:5%"><span style="cursor:pointer" data-feather="more-vertical"></span></div>': "" ) + '<div ' + (isMobile ? ' style="width:90%" ' : " ") + ' class="my-1' + (isMobile ? " cancelDrag " : " ") + 'checkbox-item-subdiv"><div class="custom-control custom-checkbox mr-sm-2"><input type="checkbox" ' + (checklist[check][0] ? 'checked="true"' : '') + ' class="custom-control-input file-checkbox" id="checkbox-' + check + '"><label class="custom-control-label' + (isMobile ? " " : " checklist-draggable ") + 'checklist-label" style="cursor:pointer; user-select: none; -webkit-user-select: none; -khtml-user-select: none; -moz-user-select: none; -ms-user-select: none;" for="checkbox-' + check + '">' + escapeHtml(checklist[check][1]) + '</label></div></div></div>')
        }
